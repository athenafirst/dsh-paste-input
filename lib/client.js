window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const TEXT_LINE_LIMIT = 50;
    const TEXT_THRESHOLD_DEFAULT = '2000';
    const textThresholdOptions = [
      { value: '0', label: '关闭' },
      { value: '1000', label: '1000 字符' },
      { value: '2000', label: '2000 字符（默认）' },
    ...
    function attachmentCopy() {
      ...
      return zh ? {
        ...
        textPasteCopy: '在输入框粘贴纯文本时，超过所选字符数（或超过 50 行）的内容不再插入输入框，而是自动转为 paste_text.txt 附件芯片——与粘贴图片同一管道，同名自动加 (2)、(3)… 序号。选择「关闭」则所有文本粘贴保持原样。',
      } : {
        ...
        textPasteCopy: 'When you paste plain text into the composer, content beyond the chosen character count (or 50 lines) becomes a paste_text.txt attachment chip instead of being inserted — same pipeline as pasted images, with (2), (3)… suffixes on name collisions. Choose Off to keep all text pastes as-is.',
      };
    }
