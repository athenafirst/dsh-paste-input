...
- **新功能**：在输入框粘贴**纯文本**时，超过阈值（默认 **2000 字符**，设置面板可调 关闭/1000/2000/5000）或超过 **50 行**的内容不再插入输入框，而是自动转为 `paste_text.txt` 附件芯片——复用粘贴文件全管道（`paste_image`/`paste_file` 同族命名，重名自动加 `(2)`、`(3)…` 序号，发送时复制进 `.dsh/tmp/attachments/`，气泡自动折叠）
